/**
 * @description
 * This is the first database migration file for the Poly-Pro Analytics project.
 * It creates the initial set of tables required for the application to function.
 * This script is designed to be run by the `golang-migrate/migrate` tool.
 *
 * The schema includes:
 * - users: For user accounts.
 * - wallets: For user's Polymarket wallet information.
 * - trades: For logging user trades.
 * - market_price_history: A native PostgreSQL partitioned table for charting data.
 * - market_sentiment_history: A native PostgreSQL partitioned table for AI-driven sentiment data.
 * - news_events: For storing news articles related to markets.
 *
 * This migration uses PostgreSQL 17's native partitioning instead of TimescaleDB,
 * which is compatible with Supabase's PostgreSQL 17 environment.
 */
